...
DEFAULT_ROLE_MAPPING_RULES: tuple[RoleMappingRule, ...] = (
    RoleMappingRule(
        request_type="new_access",
        system="jira",
        business_role="BR_JIRA_STANDARD_USER",
        system_keywords=("jira", "atlassian"),
        request_keywords=("acesso", "novo acesso", "incluir", "liberar"),
        guidance_comment="Para acesso ao Jira, informe usuario alvo, projeto/fila, nivel de acesso e justificativa.",
    ),
    RoleMappingRule(
        request_type="new_access",
        system="confluence",
        business_role="BR_CONFLUENCE_STANDARD_USER",
        system_keywords=("confluence", "wiki"),
        request_keywords=("acesso", "novo acesso", "incluir", "liberar"),
        guidance_comment="Para acesso ao Confluence, informe usuario alvo, space/area, perfil esperado e justificativa.",
    ),
    RoleMappingRule(
        request_type="new_access",
        system="github",
        business_role="BR_GITHUB_REPOSITORY_READER",
        system_keywords=("github", "repositorio", "repo"),
        request_keywords=("acesso", "novo acesso", "incluir", "liberar"),
        guidance_comment="Para acesso ao GitHub, informe usuario alvo, repositorio ou organizacao e nivel de acesso.",
    ),
    RoleMappingRule(
        request_type="new_access",
        system="sap",
        business_role="BR_SAP_STANDARD_REQUESTER",
        system_keywords=("sap", "financeiro", "fiori"),
        request_keywords=("acesso", "novo acesso", "incluir", "liberar"),
        guidance_comment="Para acesso ao SAP, informe usuario alvo, modulo, atividade esperada e justificativa.",
    ),
)
        ...
        for mapping in mappings:
            ...
            custom.append(
                RoleMappingRule(
                    ...
                    system_keywords=tuple(str(keyword).lower() for keyword in keywords if str(keyword).strip()) or (system,),
                    request_keywords=tuple(str(keyword).lower() for keyword in request_keywords if str(keyword).strip()) or ("acesso",),
    ...
    *,
    ...
    if has_guidance_language or missing_fields:
        ...
        return TicketTriageResult(
            ...
            summary="O ticket precisa de clarificacao antes de qualquer acao automatica no IGA.",
            ...
            recommended_steps=["Comentar no Jira pedindo os campos faltantes.", "Nao enviar requisicao ao IGA ate haver match confiavel da business role."],
        )

    rules = [*_custom_rules(linked_knowledge), *DEFAULT_ROLE_MAPPING_RULES]
    matching_rules = [
        rule
        for rule in rules
    ...
    recommended_steps.extend(
        [
            f"Abrir requisicao no IGA para a business role {selected_rule.business_role}.",
